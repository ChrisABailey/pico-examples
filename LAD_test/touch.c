...
uint8_t clear_sticky_touch(uint8_t ch)
{
    ...
    {  
        ...
        int count = i2c_reg_read_n(ch,addr,data,4,resp,1);  // FW resp is 5 bytes
...
/*     printf("┌────Touch┬Status─CH%d──┐\r\n",ch);
    ...
    printf("│ CH%d | %12s ",ch,gesture_to_string((int)(touch->gesture)));

    int i,b;
    for (i=0, b=touch->touches;b;b=b>>1, i++)
    {
        printf("| x= %4d | y= %4d ",touch->x[i],touch->y[i]);
    }
    printf("│ %2d Stuck │\r\n",touch->sticky_touch_count);
